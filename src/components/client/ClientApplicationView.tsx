...
const DOCUMENT_LABELS: Partial<Record<ApplicationDocument['kind'], string>> = {
  cv: 'CV',
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {documents.length === 0 ? (
            <p className="text-sm text-muted-foreground">Nog geen documenten beschikbaar.</p>
          ) : (
            <ul className="space-y-2">
              {documents.map((doc) => (
                <li key={doc.id} className="flex items-center justify-between rounded-lg border border-border px-3 py-2">
                  <div>
                    <p className="text-sm font-medium text-foreground">{DOCUMENT_LABELS[doc.kind] ?? doc.kind}</p>
