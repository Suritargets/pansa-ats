...
export async function listOnboardingOverview() {
  return guarded<(ApplicationWithCandidate & { doneSteps: number; totalSteps: number })[]>([], async () => {
    const templates = await db.select({ companyId: onboardingStepTemplates.companyId }).from(onboardingStepTemplates)
    const genericStepCount = templates.filter((t) => t.companyId === null).length
    ...
    const totalStepsForCompany = (companyId: string) => genericStepCount + (stepCountByCompany.get(companyId) ?? 0)
    ...
    return apps.map(({ application, candidate, company }) => ({
      ...
      totalSteps: totalStepsForCompany(application.companyId),
    }))
  })
}
